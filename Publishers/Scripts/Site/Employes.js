...
// Fonction de suppression d'un employe (AJAX)
$(function () {
    function RemoveClick() {
        // Get the id from the link
        var tr = $(this).parent().parent();
        var nom = tr.find("#nomEmp").html();
        var idToDelete = tr.attr("id");

        if (idToDelete != '' && confirm('Voulez-vous supprimer l\'employe:' + nom +  ' de votre panier ? ')) {
            // Perform the ajax post
            $.post("/employees/SupprEmploye", { "id": idToDelete },
            function (data) {
    ...
    // Document.ready -> link up remove event handler
    $('a[name=supprEmp]').bind('click', RemoveClick);
});
